package com.daren.chen.iot.mqtt.general.transport.server.handler.connect;
...
import com.daren.chen.iot.mqtt.general.config.RsocketServerConfig;
        ...
        switch (message.fixedHeader().messageType()) {
            case CONNECT:
                ...
                if (channelManager.checkDeviceId(clientId)) {
                    connection.write(
                        MqttMessageApi.buildConnectAck(MqttConnectReturnCode.CONNECTION_REFUSED_IDENTIFIER_REJECTED))
                        .subscribe();
                    connection.dispose();
                } else {
                    // 有密码和用户名
                    if (connectVariableHeader.hasPassword() && connectVariableHeader.hasUserName()) {
                        ...
                        if (serverConfig.getAuth().apply(mqttConnectPayload.userName(),
                            ...
                            // 用户名 密码错误
                            connection.write(MqttMessageApi
                                .buildConnectAck(MqttConnectReturnCode.CONNECTION_REFUSED_BAD_USER_NAME_OR_PASSWORD))
                                .subscribe();
                        }
                        // 有遗嘱 就保存遗嘱信息
